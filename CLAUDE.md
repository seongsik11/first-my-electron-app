...
6. **커밋 메시지 형식** — Issue 기반 개발(기본)과 예외 상황 두 가지 형식:
   - **Issue 브랜치 작업 (기본):** `type/#이슈번호 요약 메시지`
     - 예: `feat/#15 레이아웃 자동 저장 기능 구현`
     - 예: `fix/#10 왼쪽/위 방향 드래그 드롭 transform transition 제거`
   - **Issue 없는 예외 상황:** `type: summary`
     - 예: `chore: .gitignore 업데이트`
   - `type`: `feat|fix|chore|refactor|docs|test|build|ci` 중 **작업 내용에 맞게** 선택
...
## 11. Git Workflow & Agent Delegation

### 🤖 Agent Delegation Policy
- **전담 위임**: 모든 Git/GitHub 작업은 `git-workflow` 에이전트에게 **proactively delegate** 하세요.
- **정책 준수**: 에이전트는 `git-workflow.md`에 정의된 실행 지침을 최우선으로 따릅니다.

### 🚀 Fast-Track Strategy
- **효율 중심**: 새 작업을 시작할 때 `main` 브랜치로 이동하지 않습니다. 즉시 `issue/#N` 브랜치를 생성한 후, 해당 브랜치에서 `origin/main`을 베이스로 하여 최신 상태를 유지합니다.
- **직접 Push 제한**: `issue` 브랜치 외의 모든 병합(`main`, `develop`, `release`)은 반드시 GitHub Pull Request를 통해서만 진행합니다.

### 🔔 Notion Sync Rule
- **단일 트리거**: Notion 업데이트는 `issue/#N`이 `main` 브랜치에 Merge 될 때만 발생합니다. 이후의 배포 단계(develop/release) 병합 작업은 Notion에 기록하지 않습니다.
